...
db.collection("ideas").onSnapshot((snapshot) => {
  ...
  snapshot.forEach((doc) => {
    ...
    li.innerHTML = `${doc.data().idea} (Votes: All ${doc.data().votes}; Yours ${userVoteCount})`;

    const voteBtn = document.createElement("button");
    voteBtn.textContent = "Vote";
    voteBtn.disabled = userVoteCount >= 3;
    voteBtn.addEventListener("click", () => voteIdea(doc.id, doc.data().votes, doc.data().userVotes));
    ...
    retrieveVoteBtn.addEventListener("click", () => retrieveVote(doc.id, doc.data().votes, doc.data().userVotes));
    li.appendChild(retrieveVoteBtn);
